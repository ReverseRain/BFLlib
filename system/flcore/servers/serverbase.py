...
class Server(object):
    def __init__(self, args):
        ...
        self.poison_end_time = args.poison_end_time
        ...
        self.attack_clients=args.attack_clients #此处传入的仍是id list 在set clients当中变成 clients list
    ...
    def set_clients(self, other_client,attack_client):
        for i, train_slow, send_slow in zip(range(self.num_clients), self.train_slow_clients, self.send_slow_clients):
            ...
            attack= (i in self.attack_clients) if self.attack_clients else False
            clientObj = attack_client if attack else other_client
        ...
        self.attack_clients=[self.clients[i] for i in self.attack_clients] if self.attack_clients else []
    ...
    def select_clients(self,epoch=0):
        # if self.random_join_ratio:
        #     self.current_num_join_clients = np.random.choice(range(self.num_join_clients, self.num_clients+1), 1, replace=False)[0]
        # else:
        #     self.current_num_join_clients = len(self.clients)
        # selected_clients = list(np.random.choice(self.clients, self.current_num_join_clients, replace=False))
        selected_clients = self.clients if (epoch>=self.poison_start_time and epoch<self.poison_end_time) \
            else list(set(self.clients) - set(self.attack_clients))
    ...
    def receive_models(self):
        ...
        for i, w in enumerate(self.uploaded_weights):
            ...
            active_clients[i].weight = self.uploaded_weights[i]
    ...
    def evaluate(self, epoch, acc=None, loss=None):
        ...
        attack_acc = sum(stats[5])*1.0 / sum(stats[4]) if self.attack_clients else 0
